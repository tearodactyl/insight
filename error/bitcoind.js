...
Bitcoin.MAX_START_RETRY_INTERVAL = 160000;     // cap each backoff at 160s (5,10,20,40,80,160,160,...)
...
Bitcoin.DEFAULT_CONFIG_SETTINGS = [
  "rpcuser=ZeroNodeUsername",
  "rpcpassword=ZeroNodePassword",
  "rpcport=23811",
  "port=23801",
  "txindex=1",
  "insightexplorer=1",
...
Bitcoin.prototype._initDefaults = function(options) {
  ...
  this.startRetryCount = options.startRetryCount || Bitcoin.DEFAULT_START_RETRY_COUNT;
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getSaplingWitness', this, this.getSaplingWitness, 2],
    ['getSaplingBlocks', this, this.getSaplingBlocks, 2],
    ['getSupply', this, this.getSupply, 0],
    ['getZeronodestats', this, this.getZeronodestats, 0],
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './zero.conf');

  log.info('Using zero config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    spawnConfig.insightexplorer && spawnConfig.insightexplorer === 1,
    '"insightexplorer" option is required in order to use bitcore-node. ' +
      'Please add "insightexplorer=1" to your configuration and reindex an existing database if ' +
      'necessary with reindex=1'
  );

  $.checkState(
    spawnConfig.experimentalfeatures && spawnConfig.experimentalfeatures === 1,
    '"experimentalfeatures" option is required in order to use bitcore-node. ' +
      'Please add "experimentalfeatures=1" to your configuration and reindex an existing database if ' +
      'necessary with reindex=1'
  );

  $.checkState(
    spawnConfig.server && spawnConfig.server === 1,
    '"server" option is required to communicate to zerod from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from zerod. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from zerod. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in zero.conf'
  );
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/zero.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/zero.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Zero Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    // Size-up the raw ZMQ frame before parsing. Same untrusted-input door as the
    // insight-api inv handler: an empty/truncated/oversized rawtx frame can drive
    // the parser off the end of the buffer (RangeError) and, uncaught here, kills
    // the process. Reject cheaply on size, then guard the parse itself.
    if (!Buffer.isBuffer(message) ||
      ...
      log.warn('_zmqTransactionHandler: rejecting rawtx frame, bad size: ' +
        (Buffer.isBuffer(message) ? message.length + ' bytes' : typeof message));
      return;
    }
    ...
    try {
      ...
      // A bad rawtx frame must not crash the process; log and drop it. The
      // block-level indexer still picks the tx up when its block is processed.
      log.warn('_zmqTransactionHandler: failed to parse rawtx, skipping: ' +
        (err.message || err));
      return;
    }
...
// Doubling backoff for connect/tip-load retries. async.retry passes the 1-based
// retry count; we double the base interval each attempt and cap it so late
// attempts don't wait absurdly long. No Math.pow needed -- a left shift doubles.
Bitcoin.prototype._retryInterval = function(retryCount) {
  var ms = this.startRetryInterval * (1 << (retryCount - 1));
  return Math.min(ms, Bitcoin.MAX_START_RETRY_INTERVAL);
};

// Single retry policy shared by both spawn mode (_spawnChildProcess) and connect
// mode (_connectProcess) so the two cannot drift. Retries _loadTipFromNode with
// the doubling backoff above, honoring an in-flight stop. The `stopped` flag is
// reported back to the caller via the callback's second arg.
Bitcoin.prototype._retryLoadTip = function(node, callback) {
  var self = this;
  var stopped = false;
  async.retry({
    times: self.startRetryCount,
    interval: self._retryInterval.bind(self)
  }, function(done) {
    if (self.node.stopping) {
      stopped = true;
      return done();
    ...
    if (err) {
      // Give up: surface a single clear error so it lands in the journal before
      // the process unwinds. systemd (Restart=on-failure) relaunches the unit
      // after RestartSec, which re-enters this retry loop from the start.
      log.error('Failed to load tip from zerod after ' + self.startRetryCount +
...
Bitcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
  // Close any existing sub socket before creating a new one. Re-init (e.g. on a
  // reconnect or, historically, a zerod respawn) used to overwrite this property
  // and leak the old socket's fd while it stayed monitored -- the accumulation
  // behind the EMFILE crash. Closing first makes re-init idempotent on fds.
  if (node.zmqSubSocket) {
    try {
      node.zmqSubSocket.close();
    } catch (e) {
      log.warn('ZMQ: error closing stale sub socket: ' + (e.message || e));
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Zero Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned zero process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean zero process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting zero process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Zero process unexpectedly exited with code:', code);
        log.warn('Restarting zero child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Zero process restarted');
          });
    ...
    self._retryLoadTip(node, function(err, stopped) {
      exitShutdown = stopped;
      if (err) {
        return callback(err);
      }
      if (exitShutdown) {
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Zero configuration options "spawn" or "connect" are expected'));
    }
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, true, function(err, response) {
          ...
          var blockOverview = {
            ...
            finalSaplingRoot: result.finalsaplingroot,
            time: result.time,
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      if (!tx.coinbase) {
        tx.inputSatoshis += input.valueZat;
      }
      var script = null;
      var scriptAsm = null;
      if (input.scriptSig) {
      ...
      tx.inputs.push({
        ...
        satoshis: _.isUndefined(input.valueZat) ? null : input.valueZat
      });
    }
  }
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        if (tx.fOverwintered) {
          ...
          tx.nExpiryHeight = result.exiryheight;
        }
...
/**
 * Will get the witness for a given sapling output
 * @param {String} wtxid - The hex string of the transaction
 * @param {int} index - index of shielded output
 * @param {Function} callback
 */
Bitcoin.prototype.getSaplingWitness = function(wtxid, index, callback) {
  var self = this;
  this.client.getSaplingWitness(wtxid, index, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
/**
 * Will get the blocks for a given height and qty
 * @param {int} height - height of the starting block
 * @param {int} blockQty - qty of blocks fetched
 * @param {Function} callback
 */
Bitcoin.prototype.getSaplingBlocks = function(height, blockQty, callback) {
  var self = this;
  this.client.getSaplingBlocks(height, blockQty, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getSupply = function(callback) {
  var self = this;
  this.client.getSupply(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
/**
 * Will get the zeronodestats
 * @param {Function} callback
 */
Bitcoin.prototype.getZeronodestats = function(callback) {
  var self = this;
  this.client.zeronodestats(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
